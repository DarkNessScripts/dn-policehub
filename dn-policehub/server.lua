...
RegisterCommand("policehub", function(source, args)
    ...
    if xPlayer.PlayerData.job.name == "police" then
        local type = "toggle"
        TriggerEvent("dn:officers:refresh")
        
        if args[1] == "0" then
            type = "drag"
        end

        TriggerClientEvent("dn:officers:open", src, type)
    end
end)
...
RegisterServerEvent("dn:officers:refresh")
AddEventHandler("dn:officers:refresh", function()
    local new = ""
    ...
    HTML = new
    TriggerClientEvent("dn:officers:refresh", -1, HTML)
end)
